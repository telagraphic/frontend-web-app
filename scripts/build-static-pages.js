...
async function buildTemplates() {
  ...
  nunjucks.configure(viewsDir.pathname, {  // ✅ Changed from viewsPagesDir to viewsDir
    autoescape: true,
    noCache: true
  });
  ...
  for await (const file of glob.scan(pagesDir.pathname)) {
    ...
    console.log(`Rendered ${file} → ${outputPath} (${isProduction ? 'production' : 'development'} mode)`);
  }
}
