...
export class TransitionsManager {
   ...
   */
  constructor({ siteConfig, sessionStorage = window.sessionStorage, transitionType = TRANSITION_TYPES.CUSTOM }) {
   ...
   * @returns {Promise<void>}
   */
  async showTransition(element = this.overlayElement) {
    if (!this.transitionStrategy) {
      console.warn('TransitionsManager not initialized. Call init() first.');
   ...
   * @returns {Promise<void>}
   */
  async hideTransition(element = this.overlayElement) {
    if (!this.transitionStrategy) {
      console.warn('TransitionsManager not initialized. Call init() first.');
      return Promise.resolve();
    }
  ...
  async restoreTransition() {
    ...
    return this.transitionStrategy.restore();
  }
  ...
  /**
   * Store transition data in sessionStorage
   * Delegates to custom strategy if available, otherwise no-op
   * @param {Object} transitionData - Transition data object
   */
  storeTransitionData(transitionData) {
    if (this.transitionStrategy && this.transitionStrategy.storeTransitionData) {
      this.transitionStrategy.storeTransitionData(transitionData);
    }
  }

  /**
   * Update overlay with transition data from SiteConfig (route-based approach)
   * Delegates to custom strategy's prepare method
   * @param {string} route - SiteConfig route key
   * @returns {Promise<void>}
   */
  async updateTransitionOverlayFromRoute(route) {
    return this.prepareTransition(route);
  }

  /**
   * Restore transition data from sessionStorage
   * Delegates to strategy's restore method
   * @returns {Promise<void>}
   */
  async restoreTransitionData() {
    return this.restoreTransition();
  }
}
